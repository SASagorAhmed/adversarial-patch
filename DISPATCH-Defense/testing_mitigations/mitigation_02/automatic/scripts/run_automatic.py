#!/usr/bin/env python
"""Automatic DISPATCH branch — testing_mitigations/mitigation_02/automatic.

Does NOT receive true patch coordinates during localization.
Writes ONLY under this Automatic branch. LDM input is the attacked image.
"""
...
def main() -> int:
    ...
    if existing_csv.is_file():
        ...
        print(f"Resuming automatic; already completed: {sorted(done)}")
    ...
    rec_n = sum(1 for r in succ if yn(r["automatic_recovered"]))
    ious = [float(r["automatic_mask_iou"]) for r in rows]
    ctrl_n = sum(1 for r in ctrl if yn(r["automatic_control_preserved"]))
    ctrl_yes = f"{ctrl_n} / {len(ctrl)}" if ctrl else "n/a"

    def rec_by_cls(cls: str) -> str:
        sub = [r for r in succ if str(r["target_class"]).lower() == cls]
        n = sum(1 for r in sub if yn(r["automatic_recovered"]))
        return f"{n} / {len(sub)}"
    ...
    summary = {
        ...
        "person_success_recovered": rec_by_cls("person"),
        "car_success_recovered": rec_by_cls("car"),
        ...
        "min_mask_iou": float(min(ious)) if ious else None,
        "max_mask_iou": float(max(ious)) if ious else None,
    ...
    txt = f"""TESTING MITIGATION 02 — AUTOMATIC DISPATCH
===========================================
...
""" + "\n".join(
        ...
        f"mask_iou={float(r['automatic_mask_iou']):.4f}  P={float(r['automatic_mask_precision']):.4f}  "
        f"R={float(r['automatic_mask_recall']):.4f}  restored={r['restored_detected']}  "
