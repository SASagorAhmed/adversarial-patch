"""
Hyper-YOLO-to-YOLOv12-S Transfer Attack benchmark.

Reads ONLY from local source_data/ copies.
Writes ONLY under YOLOv12-S-COCO-Attack/results/.
...
from load_model import CHECKPOINT_PATH, load_yolov12s  # noqa: E402
...
def run_detector(model, device, image_path: Path, names) -> tuple[list[dict], float]:
...
def write_summary_result_txt(summary: dict, path: Path) -> None:
    ...
    lines = [
        "# YOLOv12-S TRANSFER ATTACK RESULT",
        "",
        ...
        "IoU >= 0.50",
        "",
        "Confidence and IoU drops are calculated only on both-valid targets.",
        "",
    ]
...
def write_report_md(summary: dict, path: Path) -> None:
    ...
    interpretation = (
        ...
        "These figures measure transfer behavior of existing Hyper-YOLO-N adversarial images on YOLOv12-S only; "
        "they do not establish that YOLOv12 is inherently vulnerable, nor claim universal attack effectiveness."
    )
    lines = [
        ...
        "official COCO-pretrained YOLOv12-S weights and identical official preprocessing (imgsz=640). "
        "A target is eligible only if it is validly detected on the clean image (same class, conf>=0.25, "
        "IoU>=0.50 vs selected COCO GT). Attack success requires an eligible clean detection that becomes "
        "invalid/missing on the patched image. Confidence and IoU drops are computed only for both-valid "
        "cases. No new patches were generated and no YOLOv12 training/fine-tuning was performed.",
        "",
        "## Results",
        "",
    ...
    *,
    ...
    for item in selected:
        ...
        try:
            ...
            clean_dets, clean_ms = run_detector(model, device, clean_path, names)
            ...
            patch_dets, patch_ms = run_detector(model, device, patch_path, names)
...
def write_experiment_config(device, meta) -> None:
    cfg = {
        ...
        "repository_commit": meta.get("repo_commit"),
        "framework": "official sunsmarterjie/yolov12",
...
def print_startup(device, meta) -> None:
    ...
    print("YOLOv12 repository commit:", meta.get("repo_commit"))
    print("model name:", meta["model"])
    print("checkpoint path:", meta["checkpoint"])
    print("Device:", device)
    print("================================")


def smoke_test(model, device, names, meta) -> bool:
...
def main() -> int:
    ...
    t0 = time.perf_counter()
    model, device, names, meta = load_yolov12s()
    print_startup(device, meta)
    write_experiment_config(device, meta)

    if mode == "smoke":
        return 0 if smoke_test(model, device, names, meta) else 1
    ...
    # Weighted mean conf/iou drop by both_valid_n
    conf_num = 0.0
    iou_num = 0.0
    weight = 0
    medians_conf = []
    ...
    weighted_mean_conf = (conf_num / weight) if weight else None
    weighted_mean_iou = (iou_num / weight) if weight else None
    median_of_medians_conf = statistics.median(medians_conf) if medians_conf else None

    print("\n========== YOLOv12-S TRANSFER ATTACK SUMMARY ==========")
    ...
    print()
    print("## PERSON")
    print(f"Total eligible: {tot_person_e}")
    print(f"Total success: {tot_person_s}")
    print(f"Overall ASR: {(tot_person_s / tot_person_e * 100.0) if tot_person_e else 0.0:.2f}%")
    print()
    print("## CAR")
    print(f"Total eligible: {tot_car_e}")
    print(f"Total success: {tot_car_s}")
    print(f"Overall ASR: {(tot_car_s / tot_car_e * 100.0) if tot_car_e else 0.0:.2f}%")
    print()
    print("## OVERALL")
    print(f"Total images: {tot_images}")
    print(f"Total eligible: {tot_elig}")
    print(f"Total success: {tot_succ}")
    print(f"Overall ASR: {overall_asr:.2f}%")
    print()
    print(f"weighted mean confidence drop: {weighted_mean_conf}")
    print(f"median of attack-level median confidence drops: {median_of_medians_conf}")
    print(f"weighted mean IoU drop: {weighted_mean_iou}")
    print(f"sum of both-valid counts: {both_sum}")
    print()
